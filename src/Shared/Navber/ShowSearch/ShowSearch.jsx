                    ...
                    data?.map(item => {
                        return (
                            <div className="">
                                <div className="shadow-xl border-2 rounded-[6px] border-slate-700 overflow-hidden">
                                    <div className='relative'>
                                        <figure className='rounded-t-[12px] ml-[-1px]'>
                                            <img src={(item.show.image?.medium) ? (item.show.image?.medium) : download} alt="Shoes" className='w-full h-[300px] object-cover' />
                                        </figure>
                                        ...
                                        <Link to={`/movie/${item?.show?.id}`}>
                                            <div className="info p-3 absolute top-0 left-0 w-full h-full opacity-0 hover:opacity-100 transition text-slate-200 bg-[#0000005b]">
                                                <span>Rating: {item?.show?.rating?.average} <FaStar className='star inline-block mb-1 text-yellow-600' /></span>
                                                <p>Category : {
                                                    (item?.show?.genres[0]) || (item?.show?.genres[1])
                                                        ?
                                                        <span>{item?.show?.genres[0]}, {item?.show?.genres[1]}</span>
                                                        :
                                                        item?.show?.genres[0]

                                                }</p>
                                                <p>Release Date: {item?.show?.premiered}</p>
                                ...
                                <p className='pl-2 pt-1 font-extralight text-[14px] text-slate-600 dark:text-slate-200'>{item?.show?.name}</p>
                            </div>
                        )
                    })
                }
